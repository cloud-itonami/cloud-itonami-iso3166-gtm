(ns marketentry.sim
  ...
  RGAE Precalificación-ceiling mismatch and its value-conditional
  non-trigger for a genuinely Precalificado engagement and for a
  genuinely low-value (Compra Directa-tier) engagement."
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (GTM, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up precalificacion-ceiling-mismatch) ==")
    ...
    (println "== filing/submit eng-4 (Q.500,000 Cotización-tier value, Inscripción-only -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (Q.500,000, Precalificado -- correct match, must NOT fire) ==")
