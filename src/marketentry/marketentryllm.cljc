(ns marketentry.marketentryllm
  ...
  the GTM public-sector market-entry compliance actor.
  ...
  cited), never a committed record or a real GUATECOMPRAS portal
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world GUATECOMPRAS portal
  submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)
        tier (when e (facts/required-tier (:jurisdiction e) (:contract-value-gtq e)))
        needs-precalificacion? (when tier (facts/precalificacion-required? (:jurisdiction e) tier))]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "contract-value-gtq=" (:contract-value-gtq e)
                        " tier=" tier
                        " rgae-status=" (:rgae-status e)
     ...
     :confidence (if (and e
                          (or (not needs-precalificacion?)
                              (= (:rgae-status e) :precalificado)))
                   0.9 0.3)}))
