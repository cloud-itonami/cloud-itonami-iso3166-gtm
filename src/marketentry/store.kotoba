(ns marketentry.store
  "SSoT for the GTM market-entry compliance actor, behind a `Store`
  ...
  Each engagement also carries `:rgae-status` (`:inscripcion` or
  `:precalificado`) and `:contract-value-gtq` -- the ground truth
  `marketentry.governor`'s flagship RGAE-precalificación-ceiling check
  independently verifies. This is VALUE-CONDITIONAL: only a
  registration-tier/value pair that does NOT cover the engagement's
  own declared contract value under Guatemala's own statutory
  modality-threshold ladder (LCE Arts. 17/38/43) is a violation, never
  a blanket rule against any particular `:rgae-status`.
...
(defn demo-data
  ...
  Cotización-tier contract value (Q.500,000, > Q.90,000) but is only
  Inscripción-registered (`:rgae-status :inscripcion`, no Precalificación
  on file) -> HARD hold. eng-5 declares the SAME Q.500,000 value but is
  actually Precalificado -> clean, proving the check does not
  false-positive on a legitimately-covered engagement. eng-6 is a
  Compra Directa-tier value (Q.60,000, <= Q.90,000) claimed by a
  mere-Inscripción engagement -> clean too, proving Inscripción alone
  legitimately covers the lower, non-competitive tiers (LCE Art. 43)."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Consultores Antigua S.A." :portal "GUATECOMPRAS"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :rgae-status :precalificado :contract-value-gtq 500000.0
             :drafted? false :submitted? false
             :jurisdiction "GTM" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "GUATECOMPRAS"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :rgae-status :precalificado :contract-value-gtq 500000.0
             :drafted? false :submitted? false
             :jurisdiction "ATL" :status :intake}
    "eng-3" {:id "eng-3" :operator "Servicios Quetzaltenango Ltda." :portal "GUATECOMPRAS"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 999000.0
             :rgae-status :precalificado :contract-value-gtq 500000.0
             :drafted? false :submitted? false
             :jurisdiction "GTM" :status :intake}
    "eng-4" {:id "eng-4" :operator "Obras Petén Import S.A." :portal "GUATECOMPRAS"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :rgae-status :inscripcion :contract-value-gtq 500000.0
             :drafted? false :submitted? false
             :jurisdiction "GTM" :status :intake}
    "eng-5" {:id "eng-5" :operator "Cobán Logistics S. de R.L." :portal "GUATECOMPRAS"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :rgae-status :precalificado :contract-value-gtq 500000.0
             :drafted? false :submitted? false
             :jurisdiction "GTM" :status :intake}
    "eng-6" {:id "eng-6" :operator "Suministros Sacatepéquez" :portal "GUATECOMPRAS"
             :base-fee 40000 :monthly-rate 5000 :monitoring-months 4
             :claimed-fee 60000.0
             :rgae-status :inscripcion :contract-value-gtq 60000.0
             :drafted? false :submitted? false
             :jurisdiction "GTM" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               rgae-status contract-value-gtq
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    rgae-status                           (assoc :engagement/rgae-status rgae-status)
    contract-value-gtq                    (assoc :engagement/contract-value-gtq contract-value-gtq)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/rgae-status :engagement/contract-value-gtq
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :rgae-status (:engagement/rgae-status m)
     :contract-value-gtq (:engagement/contract-value-gtq m)
