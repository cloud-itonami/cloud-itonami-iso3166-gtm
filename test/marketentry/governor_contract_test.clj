  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Consultores Antigua S.A."}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Consultores Antigua S.A." (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest precalificacion-ceiling-mismatch-is-held-and-unoverridable
  (testing "Q.500,000 Cotización-tier engagement claimed under Inscripción-only -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:precalificacion-ceiling-mismatch} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest precalificacion-ceiling-mismatch-does-not-fire-for-a-correct-match
  (testing "VALUE-CONDITIONAL: a Precalificado engagement at the SAME Q.500,000 value must NOT trigger the flagship check"
    (let [[db actor] (fresh)
          _ (assess! actor "t5bpre" "eng-5")
          _ (draft! actor "t5bpre" "eng-5")]
      (is (= :precalificado (:rgae-status (store/engagement db "eng-5"))) "fixture really is Precalificado")
      (is (= 500000.0 (:contract-value-gtq (store/engagement db "eng-5"))) "fixture really is at the SAME Cotización-tier value as eng-4")
      (let [r1 (exec-op actor "t5b" {:op :filing/submit :subject "eng-5"} operator)]
        ...
        (let [r2 (approve! actor "t5b")]
          (is (= :commit (get-in r2 [:state :disposition])))
          (is (not (some #{:precalificacion-ceiling-mismatch} (-> (store/ledger db) last :basis))))
